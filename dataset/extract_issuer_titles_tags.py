...
- NEW: 'tags' column contains exactly one ticker — the first matched ticker from tickers.txt.
...
CSV columns (strict order): url,title,text,topic,tags,date
  ...
  topic = "экономика"
  tags  = the first matched TICKER (single, uppercase)
  date  = message["date"]
...
def main():
    ...
    with open(args.out_csv, "w", encoding=encoding, newline="") as f:
        ...
        for m in messages:
            ...
            topic = "экономика"
            tags = matched[0]  # <<< единственный тикер в tags: первый совпавший из tickers.txt
